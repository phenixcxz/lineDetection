...
function Bw = Regionmerging(Gr,fBw,Bw,L,Msize)
% Bw——区域合并后的图像边沿结果；
   ...
   %%     Dom = L(x-1:x+1,y-1:y+1);
    ...
    %%        bh1 = Dom(ind(1));
        ...
        if abs(gray1-gray2) <= 10   %阈值可以调整
            Bw((edgelist{i}(:,2)-1)*nM+edgelist{i}(:,1)) = 0;
           % for h = 2:m-1  
           ...
           % end
            ...
            %for q=2:m-1
                %Bw(edgelist{i}(q,1),edgelist{i}(q,2)) = 0;  %对于连接起来的canny检测中并没有的长线段——去除
            %end
            q  = fix(m/2);
            Bw(edgelist{i}(q,1),edgelist{i}(q,2)) = 0;
    %    end
    end
end
