...
[deg1,vv1,rr1] = caleig(aa,0.1,0.35);
[deg2,vv2,rr2] = caleig(bb,0.1,0.35);
if deg1>0 && deg2>0 %如果是直线
    if abs(deg1-deg2)<10 || abs(deg1-deg2)>170
        flag = 1;
        return;
    end
end
...
for j = 1:lenb-lena
    ...
    kc1 = length(find(dirc<=10));
    kc2 = length(find(dirc>170));
    kc = kc1+kc2;
    if kc > 10
       flag = 1;
       return;
