%% 寻找横向梯度
function [grads1,grads2]=gradsDetection2(dirlist_lineT,dirlistT_lens,img,M,N,Msize)
...
%             elseif img(x,y-2)-img(x,y-3)>gap
    ...
    if grads1(m,1)/(grads1(m,1)+grads1(m,2) )>3/5 
        grads1(m,3) = 1;
    elseif grads1(m,2)/(grads1(m,1)+grads1(m,2) )> 3/5
        grads1(m,3) = 2;
    end
end
